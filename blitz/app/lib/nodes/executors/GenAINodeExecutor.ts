...
export class GenAINodeExecutor {
   ...
   * @param context - Execution context with conversation history
   * @param ragContext - Optional RAG context to enhance the response
   */
  async execute(
    message: string,
    ...
    // Use default system prompt, enhanced with RAG context if available
    const systemPrompt = this.getSystemPromptWithRAG(ragContext);
  ...
  /**
   * Gets the system prompt with optional RAG context
   */
  private getSystemPromptWithRAG(ragContext?: string): string {
    const basePrompt = `You are an intent detection system for an e-commerce chatbot. Your task is to analyze user messages and categorize them into one of these intents:
...
- If other intent: Return JSON with this structure:
{
  ...
  "data": {
    ...
    // If RAG context is provided, enhance the prompt
    if (ragContext) {
      return `${basePrompt}

IMPORTANT: You have access to relevant information from the company's knowledge base:

--- KNOWLEDGE BASE CONTEXT ---
${ragContext}
--- END CONTEXT ---

When answering general queries, USE THE ABOVE CONTEXT to provide accurate, specific answers. The context contains verified company information that should be prioritized over general knowledge. If the user's question is answered in the context, use that information directly.`;
    }
  ...
  /**
   * Gets the default system prompt for intent detection (kept for backward compatibility)
   */
  private getDefaultSystemPrompt(): string {
    return this.getSystemPromptWithRAG();
  }
