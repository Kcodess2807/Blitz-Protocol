...
export async function loadWorkflowWithConfigurations(
  ...
  const nodesWithConfig: WorkflowNodeWithConfig[] = (reactFlowState.nodes || []).map((node) => {
    ...
    // Extract RAG config from module config parameters if it's a RAG module
    let ragConfig;
    if (node.type === 'module' && node.data.moduleType === 'rag' && config.moduleConfig?.parameters) {
      const params = config.moduleConfig.parameters as any;
      ragConfig = {
        documentMode: 'paste' as const, // Default mode
        documentContent: params.pastedContent,
        uploadedFiles: params.uploadedFile ? [params.uploadedFile] : undefined,
        responseMode: params.responseMode || 'concise',
        matchThreshold: params.matchThreshold || 0.7,
