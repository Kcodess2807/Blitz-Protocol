...
import { NodeType } from '@/app/lib/types/workflow';
import { NodeConfigurationData } from '@/app/lib/db/node-configurations';
...
export async function POST(request: Request) {
  try {
    ...
    // Handle RAG module document ingestion
    if (nodeType === 'module') {
      const config = payload.config as NodeConfigurationData;
      const moduleConfig = config.moduleConfig;
      
      if (moduleConfig?.moduleType === 'rag' && moduleConfig.parameters) {
        console.log('[Save Node Config] Processing RAG module configuration...');
        
        // Check if there's content to ingest (pasted or uploaded)
        const pastedContent = (moduleConfig.parameters as any).pastedContent;
        const uploadedFile = (moduleConfig.parameters as any).uploadedFile;
        
        if (pastedContent || uploadedFile) {
          try {
            const { ingestDocument } = await import('@/app/lib/rag');
            const category = `rag-module-${payload.nodeId}`;
            
            if (pastedContent && typeof pastedContent === 'string' && pastedContent.trim()) {
              console.log('[Save Node Config] Ingesting pasted content...');
              await ingestDocument(pastedContent, {
                category,
                source: 'rag-node',
                nodeId: payload.nodeId,
                workflowId: payload.workflowId,
              });
              console.log('[Save Node Config] Pasted content ingested successfully');
            }
            
            if (uploadedFile && (uploadedFile as any).content) {
              console.log('[Save Node Config] Ingesting uploaded file:', (uploadedFile as any).name);
              await ingestDocument((uploadedFile as any).content, {
                category,
                source: 'rag-node',
                fileName: (uploadedFile as any).name,
                fileType: (uploadedFile as any).type,
                nodeId: payload.nodeId,
                workflowId: payload.workflowId,
              });
              console.log('[Save Node Config] Uploaded file ingested successfully');
            }
            
            // Mark as configured if ingestion was successful
            isConfigured = true;
          } catch (error) {
            console.error('[Save Node Config] RAG ingestion error:', error);
            // Don't fail the save, but log the error
            // The configuration will still be saved, just not marked as configured
            isConfigured = false;
          }
        }
      }
