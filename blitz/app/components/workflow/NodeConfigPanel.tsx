...
export function NodeConfigPanel({ node, allNodes = [], nodeConfig, workflowId, onUpdate, onDelete, onClose }: NodeConfigPanelProps) {
  ...
  if (node.type === 'module') {
    ...
    const moduleDescriptions: Record<ModuleType, string[]> = {
      ...
      refund: ['Initiate refunds with PSP'],
      rag: ['Retrieve context from knowledge base', 'Generate AI-powered answers using company data', 'Escalate manual reviews when required'],
    };
    ...
    const requiredAPIsMap: Record<ModuleType, string[]> = {
      ...
      rag: ['Knowledge Base (Pinecone)', 'LLM API (Groq)'],
    };
      ...
      >
        ...
        <div className="space-y-5 text-sm">
          <div>
            ...
            <input
              type="text"
              value={String((moduleConfig.parameters as any)?.customLabel || (node.data as NodeData).label || '')}
              onChange={(e) => {
                // Update module config with custom label
                setModuleConfig({
                  ...moduleConfig,
                  parameters: {
            ...
            />
            <p className="mt-1 text-xs text-gray-500">
              Custom name for this module instance (save to apply)
            </p>
          </div>
          ...
          {/* RAG Module Specific Configuration */}
          {moduleType === 'rag' && (
            <div className="space-y-4">
              <div className="rounded-2xl border border-purple-200 bg-purple-50 p-4">
                ...
                <div className="space-y-3">
                  {/* Text Input Option */}
                  <div>
                    <label className={labelClass}>Paste Content Directly</label>
                    <textarea
                      value={String((moduleConfig.parameters as any)?.pastedContent || '')}
                      onChange={(e) => {
                        setModuleConfig({
                          ...moduleConfig,
                          parameters: {
                      ...
                      placeholder="Paste your company policies, FAQs, or knowledge base content here..."
                      rows={6}
                      className={inputClass}
                    />
                    <p className="mt-1 text-xs text-gray-500">
                      Paste text content that will be used to answer queries
                    </p>
                  </div>
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        if (fileType === 'txt') {
                          ...
                          // For PDF, we'll need to handle it differently
                          // For now, just store the file info
                          alert('PDF upload will be processed. Please save configuration.');
                        }
                      }}
                      className="w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-purple-600 file:text-white hover:file:bg-purple-700"
                    />
                    <p className="mt-1 text-xs text-gray-500">
                      Upload .txt or .pdf files containing your company policies
                    </p>
                  </div>
                  
                  {moduleConfig.parameters?.uploadedFile && (moduleConfig.parameters.uploadedFile as any).name && (
                    <div className="rounded-md border border-green-200 bg-green-50 px-3 py-2">
                      <div className="flex items-center justify-between">
                        <div className="text-xs">
                          <div className="font-semibold text-green-800">
                            ✓ {(moduleConfig.parameters.uploadedFile as any).name}
                          </div>
                          <div className="text-green-600">
                            Uploaded: {new Date((moduleConfig.parameters.uploadedFile as any).uploadedAt).toLocaleString()}
                          </div>
                        </div>
                        <button
                          onClick={() => {
                  ...
                  <div>
                    <label className={labelClass}>Response Mode</label>
                    <select
                      value={String((moduleConfig.parameters as any)?.responseMode || 'concise')}
                      onChange={(e) => {
                        setModuleConfig({
                          ...moduleConfig,
                          parameters: {
                      ...
                      className={inputClass}
                    >
                      <option value="concise">Concise (2-4 lines)</option>
                      <option value="detailed">Detailed</option>
                      <option value="raw">Raw Context</option>
                    </select>
                    <p className="mt-1 text-xs text-gray-500">
                      How the RAG module should format responses
                    </p>
                  </div>
                  
                  <div>
                    <label className={labelClass}>Match Threshold (0-1)</label>
                    <input
                      type="number"
                      min="0"
                      max="1"
                      step="0.1"
                      value={Number((moduleConfig.parameters as any)?.matchThreshold || 0.7)}
                      onChange={(e) => {
                        setModuleConfig({
                          ...moduleConfig,
                          parameters: {
                    ...
                    />
                    <p className="mt-1 text-xs text-gray-500">
                      Minimum similarity score for retrieved content (higher = more strict)
                    </p>
                  </div>
                </div>
              </div>
          ...
          {/* API Configuration - Hidden for RAG modules */}
          {moduleType !== 'rag' && (
            <div className="space-y-4">
