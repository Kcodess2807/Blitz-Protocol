...
const MODULE_DEFINITIONS: Array<{
  ...
  },
  {
    moduleType: 'rag',
    label: 'RAG Module',
    description: 'Retrieve context from knowledge base and generate AI answers.',
  },
];
        ...
        <NodeConfigPanel
          ...
          onUpdate={async (nodeId, config, isConfigured) => {
            try {
              ...
              // Extract custom label from module config if present
              const customLabel = config.moduleConfig?.parameters?.customLabel;
              
              // Update node data to reflect configured status and custom label for UI display
              setNodes((current) =>
                current.map((node) =>
                  node.id === nodeId
                    ? {
              ...
              const savedConfig = nodeConfigs[nodeId];
              if (savedConfig) {
                const customLabel = savedConfig.moduleConfig?.parameters?.customLabel;
                setNodes((current) =>
                  current.map((node) =>
                    node.id === nodeId
                      ? {
